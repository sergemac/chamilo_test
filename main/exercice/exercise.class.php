        ...
        for ($answerId = 1; $answerId <= $nbrAnswers; $answerId++) {
            ...
            switch ($answerType) {
                ...
                case UNIQUE_ANSWER_NO_OPTION :
                    if ($from_database) {
                        $queryans = "SELECT answer, answer_duration FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId."' and question_id= '".$questionId."'";// Serge
                ...
                case GLOBAL_MULTIPLE_ANSWER :
                    if ($from_database) {
                        $choice = array();
                        $queryans = "SELECT answer, answer_duration FROM $TBL_TRACK_ATTEMPT WHERE exe_id = '".$exeId."' AND question_id= '".$questionId."'";
                        $resultans = Database::query($queryans);
                        while ($row = Database::fetch_array($resultans)) {
                            $ind = $row['answer'];
                            $choice[$ind] = 1;
                ...
                case MULTIPLE_ANSWER_COMBINATION_TRUE_FALSE:
                    ...
                    break;
                case MULTIPLE_ANSWER_COMBINATION: // cas du code de la route
                    if ($from_database) {
                       $queryans = "SELECT answer, answer_duration FROM ".$TBL_TRACK_ATTEMPT." where exe_id = '".$exeId."' and question_id= '".$questionId."'";
                        $resultans = Database::query($queryans);
                        while ($row = Database::fetch_array($resultans)) {
                ...
                case FILL_IN_BLANKS :
                    ...
                    while (1) {
                        ...
                        if ($from_database) {
                            $queryfill = "SELECT answer, answer_duration FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId."' AND question_id= '".Database::escape_string($questionId)."'";
                ...
                case FREE_ANSWER :
                    if ($from_database) {
                        $query  = "SELECT answer, anser_duration, marks FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId."' AND question_id= '".$questionId."'";
                ...
                case ORAL_EXPRESSION :
                    if ($from_database) {
                        $query  = "SELECT answer, answer_duration, marks FROM ".$TBL_TRACK_ATTEMPT." WHERE exe_id = '".$exeId."' AND question_id= '".$questionId."'";
                        $resq   = Database::query($query);
                        $choice = Database::result($resq,0,'answer');
                        $choice = str_replace('\r\n', '', $choice);
                        $choice = stripslashes($choice);
                ...
                case MATCHING :
                    if ($from_database) {
                        ...
                        while ($a_answers = Database::fetch_array($res_answers)) {
                            ...
                            $sql_user_answer = "SELECT answer, answer_duration, FROM $TBL_TRACK_ATTEMPT
