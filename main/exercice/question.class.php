...
abstract class Question
{
	...
	public $level;
	public $picture;
        public $chrono; // @ Author - serge Bayol serge@webin33.com
        public $chrono_media; // @ Author - serge Bayol serge@webin33.com
	...
	public function Question() {
		...
		$this->position=1;
		$this->picture='';
                $this->chrono='0000-00-00 00:00:00'; // @ Serge
                $this->chrono_media=''; // @ Serge
		$this->level = 1;
		$this->category=0;
	...
	static function read($id, $course_id = null) {
		...
		$TBL_EXERCICE_QUESTION = Database::get_course_table(TABLE_QUIZ_TEST_QUESTION);// quiz_rel_question

		$sql = "SELECT question,description,ponderation,position,type,picture,level,extra,chrono, chrono_media
		...
		if ($object = Database::fetch_object($result)) {
            ...
            if (!empty($objQuestion)) {
                ...
                $objQuestion->chrono            = $object->chrono;
                $objQuestion->chrono_media      = $object->chrono_media;
	...
	/**
	 * function chronoQuest returns chrono for the question
         * function chronoMedia returns the value for the media / chrono synchronisation
	 *
	 * @author - Serge Bayol
	 * @return - integer - chrono time (seconds)
	 */        
        function chronoQuest(){
            return $this->chrono;
            
        }
        function chronoMedia() {
            return $this->chrono_media;
        }
        function questionId(){
            return $this->id;
        }
    ...
    /**
     * updateChrono changes the exercise chrono time
     * updateChrono_media update the chronoMedia value (0 or 1)
     *
     * @author - Serge Bayol serge[@]webin33.com
     * @param - chrono question time of the question
     */
    function updateChrono($expired_date) {
        $this->chrono = $expired_date;
    }
    function updateChrono_media($chrono_media_val) {
        $this->chrono_media = $chrono_media_val;
    }   
	...
	function saveCategory($in_category) {
   	...
   	/**
	 * @author - Serge BAYOL 01-2014
	 * @param - $new question ponderation if importing from xml with mappedValue 
	 * I take the addition of the mappedValue of each answer of a question
	 * this shoud be modified if we want to impact a different score method as in quizmanager
	 */
	function savePonderation($new_ponderation) {

            // update the ponderation in the quiz_question table

			$TBL_QUIZ_QUESTION = Database::get_course_table(TABLE_QUIZ_QUESTION);
			//$category_id = Database::escape_string($in_category);
			$question_id = Database::escape_string($this->id);
			$sql = "SELECT count(*) AS nb FROM $TBL_QUIZ_QUESTION WHERE id=$question_id AND c_id=".api_get_course_int_id();
			$res = Database::query($sql);
			$row = Database::fetch_array($res);
			if ($row['nb'] > 0){
				$sql = "UPDATE $TBL_QUIZ_QUESTION SET ponderation=$new_ponderation WHERE id=$question_id AND c_id=".api_get_course_int_id();
				$res = Database::query($sql);
			} else {
                        
                                //At this time this case should not exist
  ...
  */ 
   	...
   	/**
         * function to insert the timer div
         * 
	 * @ Author - Serge BAYOL 01-2014
         * 
	 */
    ...
    */
	...
	function save($exerciseId=0) {
		...
		$level			= $this->level;
                ...
                $chrono                 = $this->chrono;
                $chrono_media           = $this->chrono_media;
		...
		if(!empty($id)) {
			$sql="UPDATE $TBL_QUESTIONS SET
					...
					level		='".Database::escape_string($level)."',
                                        chrono          ='".Database::escape_string($chrono)."',
                                        chrono_media    ='".Database::escape_string($chrono_media)."'
			...
			if ($type == HOT_SPOT_DELINEATION ) {
				$TBL_ANSWERS = Database::get_course_table(TABLE_QUIZ_ANSWER);
				$sql="INSERT INTO $TBL_ANSWERS (c_id, id, question_id , answer , correct , comment , ponderation , position , hotspot_coordinates , hotspot_type )
					   (".$c_id.", '1', '".Database::escape_string($this->id)."', '', NULL , '', '10' , '1', '0;0|0|0', 'delineation')";
				Database::query($sql);
			}
    ...
    */
	...
	function createForm (&$form, $fck_config=0) {
		...
		/*
		 * @ Serge Bayol - serge@webin33.com
		 * for the question time box show/hide
		 */
		function option_time_expired() {
		    if(document.getElementById(\'timercontrol\').style.display == \'none\')
		    {
		      document.getElementById(\'timercontrol\').style.display = \'block\';
                      document.getElementById(\'divsynchrocontrol\').style.display = \'block\';
		    } else {
		      document.getElementById(\'timercontrol\').style.display = \'none\';
                      document.getElementById(\'divsynchrocontrol\').style.display = \'none\';
		    }
	    }
		</script>';
		 ...
		 * @ Author - serge serge@webin33.com
                 ...
                 * Get expired time
		 */
                if($this->chronoQuest() != '0') {
                    $defaults['enabletimercontrol'] = 1;
                    $defaults['enabletimercontroltotalsecondes'] = (int)$this->chronoQuest();
                } else {
                    $defaults['enabletimercontroltotalsecondes'] = 0;
                }
                
                /*
		 * @ Author - serge serge@webin33.com
                 * 
                 * Get synchro to media
		 */
                if($this->chronoMedia() != '0') {
                    $defaults['enablesynchrocontrol'] = (int)$this->chronoMedia();
                } else {
                    $defaults['enablesynchrocontrol'] = 0;
                }
	 ...
	 * @ Author - serge Bayol serge@webin33.com
	 * Date - 08/01/2014
	 * add a timer option for a question
         * add an option to sync the chrono with the end of the media reading
	 */

            $diplay = 'block';
            $form->addElement('html','<div id="divtimecontrol"  style="display:'.$diplay.';">');
            $form->addElement('checkbox', 'enabletimercontrol', null, get_lang('ChronoQuest'), array('onclick' =>'option_time_expired()','id'=>'enabletimercontrol','onload'=>'check_load_time()'));
           
            $expired_date = (int)$this->chronoQuest();
            $enable_synchro = (int)$this->chronoMedia();
            $diplaySynchro = 'block';
            if (($expired_date!='0')) {
                $form->addElement('html','<div id="timercontrol" style="display:block;">');
                $form->addElement('html','<div id="divsynchrocontrol"  style="display:'.$diplaySynchro.';">');
                $form->addElement('checkbox', 'enablesynchrocontrol', null, get_lang('Synchromedia'), array('id'=>'enabletimersyncho','onload'=>'check_timer_synchro()'));
            } else {
                $form->addElement('html','<div id="timercontrol" style="display:none;">');
            }
            $form->addElement('text', 'enabletimercontroltotalsecondes',get_lang('QuestionTotalDurationSecondes'),array('style' => 'width : 35px','id' => 'enabletimercontroltotalsecondes'));
            $form->addElement('html', '</div></div>');	 
	// # timer & synchro question control   
	}
	...
	function processCreation ($form, $objExercise = null) {
		...
		$this->updateCategory($form->getSubmitValue('questionCategory'));
                $this->updateChrono($form->getSubmitValue('enabletimercontroltotalsecondes')); // @ Serge Bayol 
                $this->updateChrono_media($form->getSubmitValue('enablesynchrocontrol')); // @ Serge Bayol 
