...
function showQuestion($questionId, $only_questions = false, $origin = false, $current_item = '', $show_title = true, $freeze = false, $user_choice = array(), $show_comment = false, $exercise_feedback = null, $show_answers = false) {
    ...
    $quest_chrono         = $objQuestionTmp->chronoQuest(); // @ serge
 ...
 *  Modifications to show the intermediate label question  if there is one
 */ 
$answer = new Answer($questionId);
$answer -> read();

$quest = new Answer($questionId);
$quest->readPosition();
$quest_position = $quest->quest_position;
    	for ($answerId=1; $answerId <= $nbrAnswers; $answerId++) {
    		$answer          = $objAnswerTmp->selectAnswer($answerId);
    		$answerCorrect   = $objAnswerTmp->isCorrect($answerId);
    		$numAnswer       = $objAnswerTmp->selectAutoId($answerId);
    		...
    		if ($answerType == UNIQUE_ANSWER || $answerType == UNIQUE_ANSWER_NO_OPTION) {
    		...
    		} elseif ($answerType == MULTIPLE_ANSWER_COMBINATION) { // specific for french code de la route
    			// multiple answers
    			$input_id = 'choice-'.$questionId.'-'.$answerId;
                ...
                $labelPlace=new Answer($questionId);
                $labelPlace->readLabelPlace($answerId);    
                $theLabel = $labelPlace->questLabel;
                if(!$theLabel == ''){
                        $answer_input ='<label style="font-size:1.2em ; font-weight:bold">'.($quest_position+1).'. '.$theLabel. '</label>';
                    }else{
                        $answer_input='<label style="font-size:1.2em ; font-weight:bold"></label>';
                    }
    			$answer = Security::remove_XSS($answer, STUDENT);
...
function get_exercise_track_exercise_info($exe_id) {
    ...
    $TBL_TRACK_ATTEMPT          = Database::get_statistic_table(TABLE_STATISTIC_TRACK_E_ATTEMPT); // serge
...
function display_question_list_by_attempt($objExercise, $exe_id, $save_user_result = false) {
    ...
    if (!empty($question_list)) {
         ...
         * Date[D:M:Y] 18-02-2014
         * 
         * Param $new_answer_duration = array with the answer duration by question
         */
            
            $track_exercises = Database :: get_statistic_table(TABLE_STATISTIC_TRACK_E_EXERCICES);
            $track_attempts = Database :: get_statistic_table(TABLE_STATISTIC_TRACK_E_ATTEMPT);
            $exe_id = intval($exe_id);
            $sql_track = "SELECT tea.* FROM ".$track_exercises." as tee
                      INNER JOIN ".$track_attempts." as tea 
                          ON tee.exe_id=tea.exe_id WHERE tee.exe_id =".$exe_id;
            $result = Database::query($sql_track);
                while ($row = Database::fetch_array($result)) {
                    $new_answer_duration[$row['question_id']] = $row['answer_duration']; 
                }
            // # serge
            if ($show_results) {
                $score['result']    = get_lang('Score')." : ".show_score($my_total_score, $my_total_weight, false, true)
                        .'<span style="color:red"> | </span>'.get_lang('Answer_duration').' : '.gmdate("H:i:s",$new_answer_duration[$questionId]).' sec<span style="color:red"> | </span>';
