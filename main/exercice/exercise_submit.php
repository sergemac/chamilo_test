...
//css for movies player 
//$htmlHeadXtra[] = api_get_css(api_get_path(WEB_LIBRARY_PATH).'javascript/jplayer/libs/css/player.css');
$htmlHeadXtra[] = api_get_css(api_get_path(WEB_LIBRARY_PATH).'javascript/jplayer/libs/css/video_player.css');
//$htmlHeadXtra[] = api_get_js('jplayer/libs/js/dyn_content.js');
...
/*
 * 
 * I need to set up a cookie for each question, this one is different from the cookie for question with timer
 * @ Author - Serge Bayol serge@webin33.com
 * Date - [M:Y] 01-2014
 *
 * This script should be made in a js file
 */
$chronoquest = $objQuestionTmp->chronoQuest(); // @ Serge Bayol - Question with countdown timer 
$chrono_media = $objQuestionTmp->chronoMedia(); // @ Serge Bayol - Countdown start after the complete media reading
$question_id = $objQuestionTmp->questionId();
$start_cookie_init = "question_start_".$question_id;
$now_time = time();
if(!isset($_COOKIE[$start_cookie_init])){
    setcookie($start_cookie_init, $now_time, "0", "/");
}
if ($chronoquest != 0){
?>
<script>
  /*
   * @ Author - Serge serge@webin33.com
   * (D:M:Y) 10-02-2014
   * Script to manage the epiclock chrono for each question and the synchro with the end of the movie reading
   * Setup a cookie for the 
   */
$(document).ready(function() {
  ...
  now_chrono_seconds = (hours*3600) + (minutes*60) + seconds ; // calculate the current time in seconds to simplify the calculations
  current_question=<?php echo $current_question ?>;
})

$(document).ready(function() {
  real_left_time = <?php echo $chronoquest ; ?>;
  chrono_media = <?php echo $chrono_media ?>;
  
 function capture() {
    currentTime2   = new Date();
    hours2 	= currentTime2.getHours();
    minutes2	= currentTime2.getMinutes();
    seconds2	= currentTime2.getSeconds();
    now_chrono_seconds_2 = (hours2*3600) + (minutes2*60) + seconds2 ; // calculate the current time in seconds to simplify the calculations
    now_chrono_seconds = Number(now_chrono_seconds_2) ;
    //console.log('fin du media detectée');
    countdown_launch_end_media(real_left_time) ;
        }
        
 function set_cookie_time(){
     $.cookie('start_time_'+current_question, now_chrono_seconds, { path: '/' });
    }
    
 function countdown_launch_end_media(real_left_time) {
     //console.log('real_left_time in the coutdown function : '+real_left_time);
        $("#exercise_clock_warning").epiclock({mode: $.epiclock.modes.expire, offset: {seconds: real_left_time}}).bind('timer', function ()
                {
                    $('#exercise_clock_warning').css({color: 'red'});
                    $('#clock_warning').css({ display : 'block' });
                    $('.question_options').fadeOut("slow");
                    $('#video').fadeOut("slow");
                });
    }
    
 function get_left_time() {
     quest_start_time = $.cookie('start_time_'+current_question);
     //console.log('quest_start_time = '+quest_start_time+' / now_chrono_seconds = '+now_chrono_seconds+' time_max = '+ (Number(quest_start_time) + Number(real_left_time)));
     
//Comparison between the start time of this question and the current time and the $chronoquest value
    if((Number(quest_start_time) + Number(real_left_time)) >= now_chrono_seconds) {// the question time is not over
        //console.log('now : '+now_chrono_seconds+' / start_time : '+quest_start_time+' / time_max = '+quest_start_time + Number(real_left_time));
	real_left_time = (Number(quest_start_time)+Number(real_left_time)) - Number(now_chrono_seconds);
	return real_left_time;
      }else{//the question time is over
        //console.log('il ne reste plus de temps dispo on cache la question et la video');
            $('#exercise_clock_warning').css({display: 'none'});
            $('#clock_warning').css({ display : 'block' });
            $('.question_options').fadeOut("slow");
            $('#video').fadeOut("slow");
          }
    ...
    if(chrono_media !== 0){// synchronisation with the end of the media is required.
        //console.log('synchro avec le media requise ');
            
        if(!$.cookie('start_time_'+current_question)) { 
            //console.log('le cookie n existe pas on le cree + on lance le chrono à la fin du media');
            set_cookie_time();
            document._video = document.getElementById("video");
            document._video.addEventListener("ended", capture, false); // wait for the "ended" events then start the capture function
        }else{
            //console.log('le cookie existe on est deja passé sur cette question on doit verifier s il reste du temps');
            get_left_time();// get the left time if there is and close if no time left
            //console.log('real time left = '+real_left_time);
            countdown_launch_end_media(real_left_time);//we restart the timer with the left time
        }
    }else{// no synchro with the end media is required => launch the media reading
        //console.log('pas de synchro requise');
        
       if(!$.cookie('start_time_'+current_question)) { 
            //console.log('le cookie n existe pas on le cree + on lance le chrono immediatement');
            set_cookie_time();
            countdown_launch_end_media(real_left_time);//we start the timer with the chronoquest time (= real_left_time)
        }else{
            //console.log('le cookie existe on est deja passé sur cette question on doit verifier s il reste du temps');
            get_left_time();// get the left time if there is and close if no time left
            countdown_launch_end_media(real_left_time);//we restart the timer with the left time
...
// @ Serge
  ...
  var minutes	= currentTime.getMinutes();
  var seconds	= currentTime.getSeconds();
  var now_chrono_seconds = (hours*3600) + (minutes*60) + seconds ; // calculate the current time in seconds to simplify the calculations
answer_duration = now_chrono_seconds - Number($.cookie("start_time_"+question_id));
answer_duration = now_chrono_seconds;
console.log("answer_duration : "+answer_duration) ;
          		...
          		$("#save_for_now_"+question_id).html("'.addslashes(Display::return_icon('loading1.gif')).'");
                    $.ajax({
                        ...
                        //data: "'.$params.'&type=simple&question_id="+question_id+"&"+my_choice+"&"+hotspot+"&"+remind_list, // @ serge
                        data: "'.$params.'&type=simple&question_id="+question_id+"&"+my_choice+"&"+hotspot+"&"+remind_list+"&answer_duration="+answer_duration,
            ...
            function save_now_all(validate) {
                ...
                $.ajax({
                    ...
                    async: false,
                    url: "'.api_get_path(WEB_AJAX_PATH).'exercise.ajax.php?a=save_exercise_by_now",
                    data: "'.$params.'&type=all&"+my_choice+"&"+hotspot+"&"+free_answers+"&"+remind_list+"&answer_time="+answer_time,
                    success: function(return_value) {
                    	if (return_value == "ok") {
 ...
 * 
 */
//echo '$objQuestionTmp = '.$objQuestionTmp->chronoQuest(); // @ serge temp
//if ($objExercise->expired_time != 0 && !empty($clock_expired_time))        
if ($objQuestionTmp->chronoQuest() != 0) {
	$time_control = true;
}


if ($time_control) {
        $show_clock  = '<div id="clock_warning" style="display:none">'.Display::return_message(get_lang('ReachedTimeLimit'), 'warning').'</div>';
        $show_clock .= '<div id="exercise_clock_warning" class="well count_down"></div>';
        echo $show_clock ;
}
// # Serge       
