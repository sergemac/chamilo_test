...
/*
 * @ modified by serge bayol - serge@webin33.com
 * @ [D-M-Y] 13-02-2014
 * For the tracking the answer_duration
 */
$user_restriction = $is_allowedToEdit ? '' :  "AND user_id=".intval($student_id)." ";
$query = "SELECT attempts.question_id, answer, answer_duration FROM ".$TBL_TRACK_ATTEMPT." as attempts
...
while ($row = Database::fetch_array($result)) {
	...
	$exerciseResult[$row['question_id']] = $row['answer'];
        $answer_duration[$row['question_id']] = gmdate("H:i:s", $row['answer_duration']); // @ Serge Bayol       
}
...
foreach ($questionList as $questionId) {
	...
	} elseif ($answerType == MULTIPLE_ANSWER_COMBINATION || $answerType ==  MULTIPLE_ANSWER_COMBINATION_TRUE_FALSE) {// specific for code de la route exam
		$choice = array();
        $question_result = $objExercise->manage_answer($id, $questionId, $choice,'exercise_show', array(), false, true, $show_results, $objExercise->selectPropagateNeg()); 
        $questionScore   = $question_result['score']; // recup du score ici
        $totalScore     += $question_result['score'];
    ...
    $my_total_score  = $questionScore; // Score calculation
    ...
    $score = array();
    if ($show_results) {
        $response_time = '<div class="answer-duration">'.get_lang('Answer_duration').' : '.$answer_duration[$questionId].' sec</div>'; // Serge Bayol
	$score['result'] = "<div class='answer-duration'>".get_lang('Score')." : ".show_score($my_total_score, $my_total_weight, false, false)."</div>".$response_time;// Modified by Serge Bayol
