...
if ($inATest) {
    ...
    if (!empty($objExercise->questionList)) {
        foreach ($objExercise->questionList as $q) { // test serge
...
if ($newQuestion || $editQuestion) {
	...
	require 'question_admin.inc.php'; // here the details of each question
}
...
if (!$newQuestion && !$modifyQuestion && !$editQuestion && !isset($_GET['hotspotadmin'])) {
	...
	require 'question_list_admin.inc.php';// The list of questions
}
