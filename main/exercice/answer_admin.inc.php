...
if ($submitAnswers || $buttonBack) {
    ...
    if ($answerType == UNIQUE_ANSWER || $answerType == MULTIPLE_ANSWER || $answerType == MULTIPLE_ANSWER_COMBINATION || $answerType == GLOBAL_MULTIPLE_ANSWER) {
        if ($debug > 0) {
            echo '&nbsp;&nbsp;$answerType is UNIQUE_ANSWER or MULTIPLE_ANSWER' . "<br />\n";
        }
        ...
        for ($i = 1; $i <= $nbrAnswers; $i++) {
            ...
            if ($answerType == UNIQUE_ANSWER) {
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 4) . '$answerType is UNIQUE_ANSWER' . "<br />\n";
                }
        ...
        if ($debug > 0) {
            echo str_repeat('&nbsp;', 2) . '$answerType is FILL_IN_BLANKS' . "<br />\n";
        }
        $reponse = trim($reponse);

        if (!$buttonBack) {
            if ($debug > 0) {
                echo str_repeat('&nbsp;', 4) . '$buttonBack is not set' . "<br />\n";
            }
            if ($setWeighting) {
                $blanks = unserialize($blanks);
        ...
        if ($debug > 0) {
            echo str_repeat('&nbsp;', 2) . '$answerType is FREE_ANSWER' . "<br />\n";
        }
        ...
        if (!$buttonBack) {
            ...
            if ($setWeighting) {
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 6) . '$setWeighting is set' . "<br />\n";
                }
                // checks if the question is used in several exercises
                if ($exerciseId && !$modifyIn && $objQuestion->selectNbrExercises() > 1) {
                    $usedInSeveralExercises = 1;
            ...
            elseif (empty($free_comment)) {
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 6) . '$free_comment is empty' . "<br />\n";
                }
                $msgErr = get_lang('GiveText');
            }
            /* elseif(!ereg('\[.+\]',$reponse))
              ...
              $msgErr=get_lang('DefineBlanks');
                ...
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 6) . '$setWeighting is not set and $free_comment is not empty' . "<br />\n";
                }
...
if ($modifyAnswers) {
    ...
    if ($answerType == UNIQUE_ANSWER || $answerType == MULTIPLE_ANSWER || $answerType == GLOBAL_MULTIPLE_ANSWER) {
        if ($debug > 0) {
            echo str_repeat('&nbsp;', 2) . '$answerType is UNIQUE_ANSWER or MULTIPLE_ANSWER' . "<br />\n";
        }
        if (!$nbrAnswers) {
        ...
        if (!$submitAnswers && !$buttonBack) {
            if ($debug > 0) {
                echo str_repeat('&nbsp;', 4) . '$submitAnswers && $buttonsBack are unset' . "<br />\n";
            }
            if (!$setWeighting) {
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 6) . '$setWeighting is unset' . "<br />\n";
                }
                ...
                if ($debug > 0) {
                    echo str_repeat('&nbsp;', 6) . '$setWeighting is set and $modifyIn is unset' . "<br />\n";
                }
                $weighting = unserialize($weighting);
            }
        }
    ...
    if (!$usedInSeveralExercises) {
        ...
        if ($answerType == UNIQUE_ANSWER || $answerType == MULTIPLE_ANSWER || $answerType == MULTIPLE_ANSWER_COMBINATION || $answerType == GLOBAL_MULTIPLE_ANSWER) {
            if ($debug > 0) {
                echo str_repeat('&nbsp;', 4) . '$answerType is UNIQUE_ANSWER or MULTIPLE_ANSWER' . "<br />\n";
            }
            ?>
            <h3>
        ...
        elseif ($answerType == FILL_IN_BLANKS) {
            if ($debug > 0) {
                echo str_repeat('&nbsp;', 4) . '$answerType is FILL_IN_BLANKS' . "<br />\n";
            }
            ?>
            ...
            if ($debug > 0) {
                echo str_repeat('&nbsp;', 4) . '$answerType is FREE_ANSWER' . "<br />\n";
            }
            ?>
            <h3>
