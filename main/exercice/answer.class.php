...
/**
	...
	function read() {		
		...
		$sql="SELECT * FROM $TBL_ANSWER WHERE c_id = {$this->course_id} AND question_id ='".$questionId."' ORDER BY position";
		...
		while($object=Database::fetch_object($result)) {
			...
			$this->autoId[$i]				= $object->id_auto;
                        $this->c_id[$i]                                 = $object->c_id;
			$i++;
                        //echo 'position : '.$this->position[$i].'<br />';
		}
		$this->nbrAnswers=$i-1;
	}
	 ...
	 * @author 	Serge Bayol - serge@webin33.com
         * @return type | text & mediumint | 
         * @uses read intermediate question from the database ordered by position
         * @ - date [d:m:y] 27-02-2014
	 */
        function readPosition(){
            $TBL_QUIZ_REL_QUEST = Database::get_course_table(TABLE_QUIZ_TEST_QUESTION);
            $questionId = $this->questionId;
            $sql="SELECT question_order FROM $TBL_QUIZ_REL_QUEST WHERE c_id = {$this->course_id} AND question_id ='".$questionId."'";
            $result = Database::query($sql);
            $object=Database::fetch_object($result);
            $this->quest_position = $object->question_order;
        }
        function readLabel(){
                $TBL_QUEST_LABEL = Database::get_course_table(TABLE_QUEST_LABEL);
		$questionId = $this->questionId;
                $sql="SELECT label, position, id FROM $TBL_QUEST_LABEL WHERE c_id = {$this->course_id} AND question_id ='".$questionId."' ORDER BY position";
                $result = Database::query($sql);
                $i=1;
		// while a record is found
		while($object=Database::fetch_object($result)) {
                        $this->questLabel[$i]                           = $object->label;//label of the subsidiary question
                        $this->labelPos[$i]                             = $object->position;// position on the label (ex. if 3 then must be placed after the answer 2)
                        $this->labelId[$i]                              = $object->id; // unique id
                        $i++;
		}
                $this->nombreLabel = $i-1;
        }
        function readLabelPlace($answerId){
                $TBL_QUEST_LABEL = Database::get_course_table(TABLE_QUEST_LABEL);
		$questionId = $this->questionId;
                $sql="SELECT label, position, id FROM $TBL_QUEST_LABEL WHERE c_id = {$this->course_id} AND question_id ='".$questionId."' AND position =".$answerId;
                $result = Database::query($sql);
                $object=Database::fetch_object($result);
                        $this->questLabel                           = $object->label;//label of the subsidiary question
                        $this->labelPos                             = $object->position;// position on the label (ex. if 3 then must be placed after the answer 2)
                        $this->labelId                              = $object->id; // unique id
                }
        // # Serge
...
/**
	 ...
	 * @param	integer	Answer position
	 */
	function updateAnswers($answer,$correct,$comment,$weighting,$position,$destination) {
         ...
         * @author Serge Bayol - serge@webin33.com
         * 
         * @uses update the label(s) question
         * 
         *
        function saveLabel($labelId, $labelValue){
              
                $TABLE_QUEST_LABEL  = Database :: get_course_table(TABLE_QUEST_LABEL);
                $sql2 = "UPDATE $TABLE_QUEST_LABEL SET 
                label = 'nouveau champ ".$labelValue
                ."' WHERE id =".$labelId." AND c_id= 2";
                echo "sql2 === ".$sql2;
		Database::query($sql2);
