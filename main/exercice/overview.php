...
/*
 * check and delete the cookies 'start_time_' and 'question_start_' set for each question
 *
 * @ Author Serge Bayol - serge@webin33.com
 * @ Date 01-2014 + [D:M:Y] 13-02-2014
 * 
 */

        foreach ($_COOKIE as $key => $value) {
            $pos_quest = strpos($key, "question_start_");
            if($pos_quest===0){
                $last_pos=strrpos($key,"_");
                $quest_cookie = "question_start_".substr($key,$last_pos+1);
                setcookie ($quest_cookie, "", time() - 3600, "/");
            }
            $pos_start = strpos($key, "start_time_");
            if($pos_start===0){
                $last_pos=strrpos($key,"_");
                $start_cookie = "start_time_".substr($key,$last_pos+1);
                setcookie ($start_cookie, "", time() - 3600, "/");
            }
        }
// # serge
...
/*
 * si le temps max ($objExercise->expired_time) != 0 et si '$clock_expired_time' != de vide 
 */
