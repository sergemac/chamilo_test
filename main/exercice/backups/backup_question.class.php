...
abstract class Question
{
	...
	static function read($id, $course_id = null) {
		...
		if ($object = Database::fetch_object($result)) {
            ...
            if (!empty($objQuestion)) {
                ...
                $objQuestion->chrono_media      = $objQuestion->chrono_media;
                $objQuestion->category          = Testcategory::getCategoryForQuestion($id);
                
                echo ' question id = '.$objQuestion->id.' chrono = '.$objQuestion->chrono . ' / ';// @ serge temp
                echo 'chrono_media = '.$objQuestion->chrono_media . ' / ';// @ serge temp
                ...
                if ($result_exercise_list) {
                    while ($obj = Database::fetch_object($result_exercise_list)) {
                        ...
                        //echo 'id_exercice = ' . $obj->exercice_id;
                    }
                }
                return $objQuestion;
                $info_chrono = $object->chrono; // @ serge temp
                $chrono_media = $object->chrono_media;
            }
	...
	/**
	 * returns chrono for the question
         * returns the sync option
	 *
	 * @author - Serge Bayol
	 * @return - integer - chrono time (seconds)
	 */        
        function chronoQuest(){
            return $this->chrono;  // Serge
        }
        function chronoMedia(){
            return $this->chrono_media;  // Serge
        }
    ...
    /**
     * changes the exercise chrono time
     *
  ...
  */       
	...
	function save($exerciseId=0) {
		...
		if(!empty($id)) {
			...
			$position = $this->position;
                ...
                */
		$sql = "INSERT INTO $TBL_QUESTIONS (c_id, question, description, ponderation, position, type, picture, extra, level, chrono, chrono_media) VALUES (
					$c_id,
					...
					'".Database::escape_string($extra)."',
                                        ...
                                        '".Database::escape_string($chrono)."',
                                        '".Database::escape_string($chrono_media)."'
					)";
			Database::query($sql);
    ...
    */
	...
	function createForm (&$form, $fck_config=0) {
		 ...
		 * for the question time checkbox show/hide
		 */
		 ...
		 */
                ...
                //Get expired time
               // echo '<br />(int)$this->chronoQuest() 1377 = '.(int)$this->chronoQuest().'****<br/>';
	 ...
	 * @ Author - serge Bayol serge@webin33.com
	 * Date - 08/01/2014
	 * add a timer option for individual question
	 */
	...
	function processCreation ($form, $objExercise = null) {
		...
		$this->updateCategory($form->getSubmitValue('questionCategory'));
                $this->updateChrono($form->getSubmitValue('enabletimercontroltotalsecondes'));
