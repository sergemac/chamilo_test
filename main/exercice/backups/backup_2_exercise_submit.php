...
$htmlHeadXtra[] = api_get_css(api_get_path(WEB_LIBRARY_PATH).'javascript/jplayer/skin/blue.monday/jplayer.blue.monday.css');
$htmlHeadXtra[] = api_get_js('jplayer/libs/js/jquery.jplayer.min.js');
...
$chronoquest = $objQuestionTmp->chronoQuest(); // @ Serge Bayol
$chrono_media = $objQuestionTmp->chronoMedia(); // @ Serge Bayol

if ($chronoquest != 0){
?>
<script>
/*
 * Script to manage the epiclock chrono for a question
 *
...
$(document).ready(function() {
     ...
     * @ Author - Serge Bayol
     * 
     * Test de fin de lecture du media
     */
    if(<?php echo $chrono_media ?> != 0){
     $('#jquery_jplayer_1').bind($.jPlayer.event.ended, function(){
        ...
        alert('time 1 : '+now_chrono_seconds+' / time 2 : '+now_chrono_seconds_2);
        });
        now_chrono_seonds = now_chrono_seconds_2 ;
    }
   ...
   if(!$.cookie('start_time_'+<?php echo $current_question ?>)) {
    	...
    	$.cookie('start_time_'+<?php echo $current_question ?>, now_chrono_seconds);
		
   		// launch the epiclock chrono
	   ...
	   if((Number(quest_start_time) + Number(<?php echo $chronoquest ?>)) >= now_chrono_seconds) {
		   ...
		   //Over time we hide the question and give and overtime message
