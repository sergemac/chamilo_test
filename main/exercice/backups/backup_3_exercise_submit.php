...
$(document).ready(function() {
 ...
 function capture() {
   ...
   start_chrono() ;
    ...
    if(<?php echo $chrono_media ?> != 0){// synchronisation with the end of the media is required.

    document._video = document.getElementById("video");
    document._video.addEventListener("ended", capture, false);
  //console.log('doc video : '+document._video);
    }
 
 function start_chrono() {
     //alert('ended');
   // set cookies to remind the start time on each question which will be compare to now() and the $chronoquest value
   if(!$.cookie('start_time_'+<?php echo $current_question ?>)) {
	  	// the cookies doesn't exist => first attempt
                console.log('the cookies doesn t exist => first attempt');
    	$.cookie('start_time_'+<?php echo $current_question ?>, now_chrono_seconds, { path: '/' });
             
             //$('#jquery_jplayer_1').one($.jPlayer.event.ended, function() {// Start the chrono after the end of the media reading
                 //alert('Attention le chrono va demarrer');
   		// Give alerts infos : red numbers, message, AND fadeout the question div and video div
		$("#exercise_clock_warning").epiclock({mode: $.epiclock.modes.expire, offset: {seconds: <?php echo $chronoquest ; ?>}}).bind('timer', function ()
                {
	   ...
	   // Cookie for this question already exist => the user came back on this question or reload this page. Calcuate if there is remaining time
	   quest_start_time = $.cookie('start_time_'+<?php echo $current_question ?>);
	   console.log('quest_start_time = '+quest_start_time+' / now_chrono_seconds = '+now_chrono_seconds+' time_max = '+ (Number(quest_start_time) + Number(<?php echo $chronoquest ?>)));
	   
	   //Comparison between the start time of this question and the current time and the $chronoquest value
	   if((Number(quest_start_time) + Number(<?php echo $chronoquest ?>)) >= now_chrono_seconds) {
		   //console.log('inf a chronoquest : '+now_chrono_seconds+'/'+quest_start_time+' time_max = '+quest_start_time + <?php echo $chronoquest ?>);
		   // Launch epiclclock with the left time
		   		time_left = (Number(quest_start_time)+Number(<?php echo $chronoquest ?>)) - Number(now_chrono_seconds);
			//console.log('time left = '+time_left+' typeof : '+time_left);
		   		$("#exercise_clock_warning").epiclock({mode: $.epiclock.modes.expire, offset: {seconds:  time_left }}).bind('timer', function ()
                {
                    $('#exercise_clock_warning').css({color: 'red'});
                    $('#clock_warning').css({ display : 'block' });
                    $('.question_options').fadeOut("slow");
		     ...
		     console.log('time_left : '+time_left);
                    ...
                    // Overtime we hide the question and give and overtime message 
