...
function import_exercise($file) {
	...
	$exercise->save(); // @serge => function is in exercice.class.php ligne 680
	$last_exercise_id = $exercise->selectId();
        echo 'last exercice id : ' . $last_exercise_id;
        //print_r($exercice_info);
	if (!empty($last_exercise_id)) {
		...
		foreach ($exercise_info['question'] as $key => $question_array) {
			...
			//2.create question
			$question = new Ims2Question(); // qti2_classes.php ligne 38
			$question->type = $question_array['type'];
			$question->setAnswer();
			$question->updateTitle($question_array['title']); // question ...
                        
                      /**
                       * @ param - update the description with image path
                       * 
                       * @ Author - Serge <serge@citypro-campus.info> 12-2013
                       * 
                       */  
                        $main_path =  api_get_path(WEB_PATH).'courses/' ;
                     //echo 'main path : '.$main_path.'<br />'; 
                        $current_image_path = $exercise_info['question'][$key]['attached_file_url'];
                        $current_image = strstr($current_image_path,'/');
                     //echo 'current image = '.$current_image.'<br />';
                        $image_folder = strstr($current_image_path,'/',true).'/document/learning_path';// usual image path
                     //echo 'image folder : '.$image_folder.'<br />';
                        $new_image_path = '<img src="'.$main_path.$image_folder.$current_image.'"/>';
                     //echo 'image path : '.$main_path.$image_folder.$current_image.'<br />';
                        
                        if($exercise_info['question'][$key]['attached_file_url']){
                        $question->updateDescription($new_image_path);
                        /* #serge update */
                        }
			...
			$last_question_id = $question->selectId();
                       ...
                       * 
                       * @ Author - Serge <serge@citypro-campus.info> 12-2013
                       * @ param - update the question category
                       * 
                       */ 
                        $current_cat = $exercise_info['question'][$key]['category'];
                        $my_question_id = $question->selectId();
                        $question->saveCategory($current_cat);
                        /*
                         * # Category
                         */
			...
			foreach ($question_array['answer'] as $key => $answers) {
				...
				$answer->new_weighting[$i] = $question_array['weighting'][$key]; // ponderation de chaque reponse
                                $new_question_ponderation += $question_array['weighting'][$key];
			...
			$answer->save();
                      ...
                      /**
                       * 
                       * @ Author - Serge <serge@citypro-campus.info> 01-2014
                       * @ param  - update the question ponderation
                       * 
                       */ 
                                $question->savePonderation($new_question_ponderation); // @ Author - Serge - update the question weight
                                unset($new_question_ponderation); // @ serge
                       /* #serge ponderation */
...
function parse_file($exercisePath, $file, $questionFile) {
	...
	$questionFilePath = $questionTempDir . $questionFile;
        ...
        //echo "questionFilePath = ".$questionFilePath;//@serge
	...
	//used global variable start values declaration :
        ...
        $record_item_body = false; //chamilo config
		...
		case 'ASSESSMENTITEM' :
			{
				//retrieve current question
                                // retrouve la question courrante
		...
		case 'RESPONSEDECLARATION' :
			{
				...
				$current_answer_id = $attributes['IDENTIFIER'];
                                ...
                                //@ author Serge bayol
                                // retrive the id category of the question
                                ...
                                $current_answer_category = $attribute['CATEGORY'];
			}
			break;
...
function endElement($parser, $name) {
	...
	if ($record_item_body && (!in_array($current_element, $non_HTML_tag_to_avoid))) {
		...
		$current_question_item_body .= "</" . $name . ">"; //= titre de la question
	}
