...
function exercise_attempt( $score, $answer, $question_id, $exe_id, $position, $exercise_id = 0, $nano = null) {
    ...
    if (!empty($question_id) && !empty($exe_id) && !empty($user_id)) {
         ...
         * 
         * For the answer_duration calculation & recording & cookies mangement
         */
        if(isset($_COOKIE["question_start_".$question_id])){
			$answer_duration=(time())-($_COOKIE["question_start_".$question_id]);
		}else{
			$answer_duration=0;
			}
        $sql = "INSERT INTO $TBL_TRACK_ATTEMPT (exe_id, user_id, question_id, answer, marks, course_code, session_id, position, tms, filename, answer_duration)
                  VALUES (
                  ".$exe_id.",
                  ".$user_id.",
                   '".$question_id."',
